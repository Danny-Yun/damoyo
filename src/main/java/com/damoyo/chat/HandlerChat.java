...
public class HandlerChat extends TextWebSocketHandler{
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		case "CMD_ENTER":
			...
			map.put("name", mapReceive.get("name"));
			sessionList.add(map);
			String name = (String) mapReceive.get("name");
			...
			for (int i = 0; i < sessionList.size(); i++) {
				...
				if(room_id.equals(mapReceive.get("room_id"))) {
					...
					mapToSend.put("msg", name + "님이 닙장했습니다.");
		...
		case "CMD_MSG_SEND":
			...
			for (int i = 0; i < sessionList.size(); i++) {
				...
				if (room_id.equals(mapReceive.get("room_id"))) {
					...
					// 메제시를 실어서
					mapToSend.put("msg", mapReceive.get("name") + " : " + mapReceive.get("msg"));
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		for (int i = 0; i < sessionList.size(); i++) {
			...
			System.out.println("사용자 연결 헤제 : "+map);
		...
		for (int i = 0; i < sessionList.size(); i++) {
			...
			if(room_id.equals(current_room_id)) {
				...
				mapToSend.put("msg", name + "님이 퇴장했습니다.");
